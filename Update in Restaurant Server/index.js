...
//Find a restaurant and update its data

const updateRestaurant = async (restaurantId, dataToUpdate) => {
    try{
        const updatedRestaurant = Restaurants.findByIdAndUpdate(restaurantId, dataToUpdate, {new: true})
...
app.post('/restaurants/:restaurantId', (req, res) => {
    try{
        const updatedRestaurant = updateRestaurant(req.params.restaurantId, req.body)
        ...
        res.status(500).json({error: 'Error in updating restaurants.'})
    }
})
